# Pre-registered fold-4 comparison of completed full-league GAM and CAR fits.
#
# Audit mode verifies model artifacts and metadata without reading any held-out
# make/miss outcomes. Run mode opens fold 4 once, after those checks pass. The
# outcome loader rejects fold 5 before Arrow can select SHOT_MADE_FLAG.
...
if (length(args) != 2L) {
  stop(
    paste(
      "Usage: Rscript R/spatial_full_league_fold4_comparison.R",
      "<season> <audit|run>"
    ),
    call. = FALSE
  )
...
if (!identical(season, "2025-26")) {
  stop("The frozen comparison is registered only for 2025-26", call. = FALSE)
}
...
check_or_stop <- function(area, check, condition, detail) {
  ...
  if (!passed) {
    stop("Frozen comparison check failed: ", check, " — ", detail, call. = FALSE)
  }
  invisible(TRUE)
}
...
write_new_atomic_parquet <- function(table, path) {
  ...
  if (file.exists(path)) {
    stop("Refusing to overwrite an existing result: ", path, call. = FALSE)
  }
  temporary <- tempfile(
...
verify_artifacts <- function(metadata, grid) {
  ...
  expected_md5 <- c(gam = EXPECTED_GAM_FIT_MD5, car = EXPECTED_CAR_FIT_MD5)
  check_or_stop(
    "artifacts", "fitted_model_md5_hashes_match",
    identical(observed_md5, expected_md5),
    "both serialized fitted-model MD5 values match their recorded values"
  )

  # Only small checkpoints are loaded. The multi-gigabyte fitted models are not
  # needed for evaluation because their atomic checkpoints already contain the
  # frozen draw-averaged prediction surfaces and uncertainty intervals.
  ...
  check_or_stop(
    ...
    "both atomic completions belong to the frozen season, split, and inputs"
  )
  check_or_stop(
    "artifacts", "fit_references",
    identical(gam$fit_md5, EXPECTED_GAM_FIT_MD5) &&
      identical(car_completion$serialized_model_md5, EXPECTED_CAR_FIT_MD5),
    "checkpoint fit references match the verified fitted models"
  )
  ...
  check_or_stop(
    ...
    grepl("cbind\\(makes, attempts - makes\\)", gm$formula[[1]]) &&
      ...
      identical(gm$method[[1]], "aggregated_exact_nondiscrete_two_worker_psock") &&
      gm$worker_count[[1]] == 2L && gm$coefficient_count[[1]] == 6360L &&
      gm$smooth_count[[1]] == EXPECTED_PLAYERS &&
      gm$smoothing_parameter_count[[1]] == 1L,
    "grouped binomial, player intercepts/smooths, k=20, shared id=1, fREML, exact two-worker fit"
  )
  check_or_stop(
    ...
    "binomial replicated besagproper2 with frozen priors and shared hyperparameters"
  )
  check_or_stop(
    "models", "completed_prevalidation_checks",
  ...
  validation_metadata <- metadata$joined |>
    filter(fold == VALIDATION_FOLD, PLAYER_ID %in% metadata$eligible$PLAYER_ID)
  ...
  check_or_stop(
    ...
    "both fits record 116,955 shots and 19,475 observed player-cells"
  )
  ...
  car_probabilities <- car$result$probabilities |>
    arrange(PLAYER_ID, cell_id)
  probability_keys <- c("PLAYER_ID", "cell_id")
  check_or_stop(
    ...
    nrow(gam_probabilities) == EXPECTED_LATTICE_ROWS &&
      ...
      identical(gam_probabilities[probability_keys], car_probabilities[probability_keys]) &&
      ...
      identical(
        gam_signature$lattice |> select(all_of(probability_keys)) |> as_tibble(),
        gam_probabilities |> select(all_of(probability_keys))
      ),
    "same 318 players and 156 cells in the same 49,608-row order"
  )
  check_or_stop(
    ...
    "all saved draw-averaged probabilities are finite and within [0,1]"
  )

  gam_sparse <- gam$result$sparse_intervals |>
    mutate(model = "GAM", grid_width = GRID_WIDTH, .before = 1) |>
    arrange(PLAYER_ID)
  car_sparse <- car$result$sparse |>
    mutate(validation_attempts = NA_integer_) |>
    select(model, grid_width, PLAYER_ID, validation_attempts,
           interval_lower, interval_upper, interval_width) |>
    arrange(PLAYER_ID)
  check_or_stop(
    ...
    "both saved interval sets cover the same 80 training-defined sparse players"
  )
  check_or_stop(
    "seal", "pre_evaluation_outcomes_sealed",
    !"SHOT_MADE_FLAG" %in% names(training_metadata) &&
      !"SHOT_MADE_FLAG" %in% names(validation_metadata),
    "artifact audit used metadata only; fold 4 and fold 5 outcomes remain unopened"
  )

  list(
    gam_probabilities = gam_probabilities,
    car_probabilities = car_probabilities,
    gam_sparse = gam_sparse,
    car_sparse = car_sparse,
    gam_metrics = gm,
    car_metrics = cm,
...
# This is the only outcome loader in the script. It accepts fold 4 exactly and
# rejects fold 5 before opening the Arrow dataset or selecting its outcome.
read_fold4_outcomes_once <- local({
  already_read <- FALSE
  function(folds, eligible_players) {
    if (already_read) {
      stop("Fold 4 outcomes may be opened only once", call. = FALSE)
    }
    requested_fold <- VALIDATION_FOLD
    if (!identical(requested_fold, 4L) || requested_fold == SEALED_TEST_FOLD) {
      stop("SEALED TEST SAFEGUARD: fold 5 outcomes cannot be requested", call. = FALSE)
    }
    ...
    already_read <<- TRUE
    check_or_stop(
      "seal", "only_fold4_outcomes_opened",
      identical(sort(unique(outcomes$fold)), VALIDATION_FOLD) &&
        !any(outcomes$fold == SEALED_TEST_FOLD),
      "the single held-out outcome read contains fold 4 only"
    )
    check_or_stop(
      "data", "fold4_outcome_values",
      !anyNA(outcomes$SHOT_MADE_FLAG) &&
        all(outcomes$SHOT_MADE_FLAG %in% c(0L, 1L)),
      "fold-4 make/miss values are complete and binary"
    )
    outcomes
  }
})
...
calibration_table <- function(scored, model, probability_column) {
  ...
  scored |>
    transmute(
      GAME_ID, PLAYER_ID, validation_row_id, SHOT_MADE_FLAG,
      probability = !!probability_symbol
    ) |>
    arrange(probability, GAME_ID, PLAYER_ID, validation_row_id) |>
...
if (mode == "audit") {
  audit_summary <- tibble(
    ...
    saved_sparse_players = EXPECTED_SPARSE_PLAYERS,
...
if (dir.exists(result_dir) && length(list.files(result_dir)) > 0L) {
  stop("Result directory already contains files; refusing to overwrite", call. = FALSE)
}

validation <- read_fold4_outcomes_once(folds, metadata$eligible$PLAYER_ID) |>
  arrange(GAME_ID, PLAYER_ID, LOC_X, LOC_Y, SHOT_MADE_FLAG) |>
  mutate(validation_row_id = row_number())
validation_metadata_keys <- artifacts$validation_metadata |>
  select(all_of(METADATA_COLUMNS), fold) |>
  arrange(GAME_ID, PLAYER_ID, LOC_X, LOC_Y)
validation_outcome_keys <- validation |>
  select(all_of(METADATA_COLUMNS), fold) |>
  arrange(GAME_ID, PLAYER_ID, LOC_X, LOC_Y)
check_or_stop(
  "seal", "fold4_rows_match_preopened_metadata",
  identical(as.data.frame(validation_metadata_keys),
            as.data.frame(validation_outcome_keys)),
  paste(nrow(validation), "fold-4 outcome rows match metadata audited before opening")
)

validation <- assign_grid_cells(validation, grid)
...
check_or_stop(
  ...
  nrow(scored) == nrow(validation) &&
    !anyNA(scored$gam_probability) && !anyNA(scored$car_probability),
  "every fold-4 shot has exactly one GAM and one CAR probability"
)
check_or_stop(
  ...
  "all evaluated probabilities are finite and within [0,1] before clipping"
)
check_or_stop(
...
calibration_bins <- bind_rows(
  ...
  mutate(
    ...
    season = season,
    evaluation_fold = VALIDATION_FOLD,
    .before = 1
  ) |>
...
sparse_actuals <- scored |>
  filter(PLAYER_ID %in% artifacts$gam_sparse$PLAYER_ID) |>
  summarise(
    validation_attempts = n(), observed_makes = sum(SHOT_MADE_FLAG),
    .by = PLAYER_ID
  ) |>
  arrange(PLAYER_ID)
sparse_intervals <- bind_rows(
  artifacts$gam_sparse,
  artifacts$car_sparse
) |>
...
check_or_stop(
  ...
  nrow(sparse_intervals) == 2L * EXPECTED_SPARSE_PLAYERS &&
    ...
    !anyNA(sparse_intervals$validation_attempts) &&
  ...
  "two complete, finite, feasible interval rows for each of 80 sparse players"
)
...
check_or_stop(
  ...
  "2,000 finite paired resamples of all 246 fold-4 games"
)
...
model_metrics <- tibble(
  ...
  left_join(
    calibration_bins |>
      summarise(
        ...
        calibration_max_absolute_gap = max(absolute_gap),
        .by = model
      ),
    by = "model"
  ) |>
  ...
  left_join(
    sparse_intervals |>
      summarise(
        ...
        sparse_average_interval_width = mean(interval_width),
        .by = model
      ),
    by = "model"
  ) |>
  mutate(
    season = season,
    evaluation_fold = VALIDATION_FOLD,
    validation_games = n_distinct(scored$GAME_ID),
...
decision_reason <- if (identical(evidence_classification, "favors_car")) {
  ...
  paste(
    ...
    "the fold-4 evidence is therefore inconclusive."
  )
} else {
  paste(
...
decision <- tibble(
  season = season,
  evaluation_fold = VALIDATION_FOLD,
  result_scope = "full_league_fold4_fixed_grid_preliminary",
  evidence_classification = evidence_classification,
  primary_classification = primary_classification,
  car_calibration_materially_worse = car_calibration_materially_worse,
  sparse_uncertainty_role = paste(
    "descriptive only; saved intervals support the full-sample coverage/width",
    "check but not the pre-registered whole-game uncertainty bootstrap"
  ),
...
pre_evaluation_commit <- system2("git", c("rev-parse", "HEAD"), stdout = TRUE)
check_or_stop(
  "reproducibility", "pre_evaluation_commit_recorded",
  length(pre_evaluation_commit) == 1L &&
    grepl("^[0-9a-f]{40}$", pre_evaluation_commit),
  pre_evaluation_commit
)
manifest <- tibble(
  season = season,
  scope = "full_league_fold4_fixed_grid_preliminary",
  pre_evaluation_commit = pre_evaluation_commit,
  ...
  uncertainty_rule = paste(
    "training-defined bottom quartile; 90% posterior-predictive coverage and",
    "mean width; descriptive because saved draws do not support game resampling"
  ),
  ...
  interpretation_rule = paste(
    ...
    "otherwise practically tied"
  ),
  fold4_outcomes_read = TRUE,
  fold5_outcomes_read = FALSE,
...
check_or_stop(
  ...
  all(checks$passed),
  paste(nrow(checks), "checks passed before result publication")
)
checks <- bind_rows(check_records) |>
  mutate(
    season = season,
