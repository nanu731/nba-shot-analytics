...
make_shot_plan <- function(player_shots, player_lattice, source_order,
                           supported, relocation_available, marker_count,
                           marker_weights) {
  ...
  marker_count <- min(as.integer(marker_count), nrow(ordered))
  if (marker_count > 0L) ordered <- dplyr::slice_head(ordered, n = marker_count)
  ordered$move_order <- seq_len(nrow(ordered))
  if (!relocation_available || marker_count == 0L || nrow(ordered) == 0L) {
    return(player_shots |>
  ...
  destination_indices <- which(marker_weights > 0)
  target_assert(all(supported[destination_indices]),
                "a marker weight uses an unsupported destination")
  sequence <- target_destination_sequence(
    nrow(ordered), destination_indices, marker_weights
  )
...
calculate_players <- function(input, shots, surface, probability_draws) {
  ...
  for (player_index in seq_along(input$player_ids)) {
    ...
    evidence_status <- target_evidence_status(supported_count)
    ...
    destination_capacity <- numeric(CELLS_PER_PLAYER)
    destination_capacity[supported] <- pmax(
      0, DESTINATION_CAP - player_lattice$baseline_share[supported]
    )
    total_destination_capacity <- sum(destination_capacity)
    relocation_available <- supported_count > 0L &&
      source_share > TOLERANCE && total_destination_capacity > TOLERANCE
    availability_reason <- if (supported_count == 0L) {
      "no_supported_destinations"
    } else if (total_destination_capacity <= TOLERANCE) {
      "no_positive_supported_capacity"
    } else if (source_share <= TOLERANCE) {
    ...
    for (slider_index in seq_along(SLIDERS)) {
      ...
      if (relocation_available) {
        allocation <- target_capped_allocation(
          player_lattice$baseline_share, supported, requested,
          source_share, DESTINATION_CAP
        )
        removal <- target_remove_mass(
          player_lattice$baseline_share, source_order, allocation$actual
        )
        ...
        target_assert(
          all(allocation$added[!supported] == 0),
          "an unsupported destination received relocated mass"
        )
        ...
        slider_objects[[slider_index]] <- list(
          ...
          destination_allocation = unname(lapply(which(supported), function(index) {
            list(
              cell_id = as.integer(player_lattice$cell_id[[index]]),
              added_share = allocation$added[[index]],
              final_share = relocated_share[[index]]
            )
          }))
        )
        if (slider_index == length(SLIDERS)) {
          relocated_25_draws <- relocated_draws
          allocation_25 <- allocation
        }
      } else {
        slider_objects[[slider_index]] <- list(
          requested_share = requested,
    ...
    if (relocation_available) {
      ...
      if (slider_objects[[length(SLIDERS)]]$actual_relocated_share <
            max(SLIDERS) - TOLERANCE) {
        capped_players <- capped_players + 1L
      }
    } else {
      score <- list(point = NA_real_, lower_90 = NA_real_, upper_90 = NA_real_)
    }

    marker_count <- if (relocation_available) {
      ceiling(slider_objects[[length(SLIDERS)]]$
                actual_relocated_attempt_equivalents - TOLERANCE)
    } else {
      0L
    }
    marker_weights <- numeric(CELLS_PER_PLAYER)
    if (relocation_available && allocation_25$actual > TOLERANCE) {
      marker_weights <- allocation_25$added / allocation_25$actual
    }
    shot_plan <- make_shot_plan(
      player_shots, player_lattice, source_order, supported,
      relocation_available, marker_count, marker_weights
    )
    ...
    if (relocation_available) {
      ...
      target_assert(length(potential) == marker_count,
                    "move order does not match the feasible marker count")
    } else {
      target_assert(length(potential) == 0L,
                    "unavailable player received movable shots")
    }
    ...
    players[[player_index]] <- list(
      ...
      score_available = relocation_available,
      ...
      supported_destination_count = supported_count,
      maximum_destination_share = DESTINATION_CAP,
      total_supported_destination_capacity = total_destination_capacity,
...
index_entry <- function(player) {
  list(
    ...
    supported_destination_count = player$supported_destination_count,
...
build_bundle <- function(root, result, pre_result_commit, source_hashes) {
  ...
  available <- sum(vapply(ordered, `[[`, logical(1), "relocation_available"))
  statuses <- vapply(ordered, `[[`, character(1), "evidence_status")
  ...
  manifest <- list(
    ...
    counts = list(
      ...
      relocation_available = available,
    ...
    relocation = list(
      ...
      destination_rule = paste(
        ...
        "player's current mix; one supported destination is sufficient."
      ),
      destination_allocation = paste(
        "proportional to existing usage among supported destinations,",
        "with redistribution around a universal 50% final-share cap"
      ),
      maximum_final_destination_share = DESTINATION_CAP,
    ...
    summary = list(
      ...
      players_below_requested_25_percent = result$capped_players,
...
validate_bundle <- function(root) {
  ...
  total_shots <- total_cells <- available <- 0L
  status_counts <- c(
    insufficient_evidence = 0L,
    single_destination = 0L,
    multiple_destinations = 0L
  )
  for (entry in entries) {
    ...
    player_raw <- fromJSON(player_path, simplifyVector = FALSE)
    player <- fromJSON(player_path, simplifyVector = TRUE, flatten = TRUE)
    target_assert(
      identical(entry$evidence_status, player$evidence_status) &&
        identical(
          entry$supported_destination_count,
          player$supported_destination_count
        ) &&
        identical(entry$relocation_available, player$relocation_available) &&
        identical(entry$availability_reason, player$availability_reason) &&
        identical(entry$score_available, player$score_available),
      "player index evidence fields differ from the player payload"
    )
    ...
    target_assert(
      player$evidence_status %in% names(status_counts) &&
        identical(
          player$evidence_status,
          target_evidence_status(player$supported_destination_count)
        ),
      "evidence status does not match the supported destination count"
    )
    status_counts[[player$evidence_status]] <-
      status_counts[[player$evidence_status]] + 1L
    supported_ids <- player$heatmap_cells$cell_id[
      player$heatmap_cells$supported_destination
    ]
    baseline_shares <- setNames(
    ...
    for (slider_index in seq_along(player_raw$sliders)) {
      ...
      expected_allocation <- target_capped_allocation(
        unname(baseline_shares),
        player$heatmap_cells$supported_destination,
        slider_raw$requested_share,
        source_capacity,
        DESTINATION_CAP
      )
      if (!isTRUE(player$relocation_available)) {
        target_assert(
          slider_raw$actual_relocated_share == 0 && length(allocation) == 0L,
          "an unavailable player has a public destination allocation"
        )
        next
      }
      allocation_ids <- vapply(allocation, `[[`, integer(1), "cell_id")
      added <- vapply(allocation, `[[`, numeric(1), "added_share")
      final <- vapply(allocation, `[[`, numeric(1), "final_share")
      target_assert(
        all(allocation_ids %in% supported_ids) &&
          abs(slider_raw$actual_relocated_share - expected_allocation$actual) <=
            TOLERANCE &&
          abs(sum(added) - slider_raw$actual_relocated_share) <= TOLERANCE &&
          max(abs(
            added - expected_allocation$added[
    ...
    if (IS_V3_RELEASE && identical(player$player_id, "1641705")) {
      target_assert(
        identical(player$observed_attempts, 1080L) &&
          ...
          identical(player$supported_destination_count, 1L) &&
          abs(max(player$sliders$actual_relocated_share) - 0.225) <= TOLERANCE &&
          max(unlist(lapply(
            player_raw$sliders,
            function(slider) vapply(
              slider$destination_allocation, `[[`, numeric(1), "final_share"
            )
          ))) <= DESTINATION_CAP + TOLERANCE,
        "Wembanyama regression result changed"
      )
    }
  }
  ...
  target_assert(sum(status_counts) == EXPECTED_PLAYERS,
  ...
  target_assert(identical(manifest$counts$shots, EXPECTED_SHOTS) &&
                  ...
                  identical(manifest$counts$relocation_available, available) &&
                  identical(
                    manifest$counts$insufficient_evidence,
                    unname(status_counts[["insufficient_evidence"]])
                  ) &&
                  identical(
                    manifest$counts$single_destination,
                    unname(status_counts[["single_destination"]])
                  ) &&
                  identical(
                    manifest$counts$multiple_destinations,
                    unname(status_counts[["multiple_destinations"]])
                  ),
...
if (mode == "verify") {
  ...
  target_assert(file.exists(completion_path), "v3 completion marker is missing")
...
first <- tempfile("v3-first-", cache_dir)
second <- tempfile("v3-second-", cache_dir)
target_assert(dir.create(first), "could not create first v3 staging directory")
target_assert(dir.create(second), "could not create second v3 staging directory")
...
target_assert(identical(first_check$files, second_check$files) &&
                identical(first_check$hashes, second_check$hashes),
              "two v3 builds are not byte-for-byte identical")
unlink(second, recursive = TRUE)
dir.create(dirname(bundle_dir), recursive = TRUE, showWarnings = FALSE)
target_assert(file.rename(first, bundle_dir), "could not publish v3 atomically")
final_check <- validate_bundle(bundle_dir)
target_assert(identical(first_check$hashes, final_check$hashes),
              "published v3 differs from its verified staging build")
